	...
	main(int argc, char *argv[])
{
   ...
   if(argc <= nreqarg){
      ...
      cout<<"Usage> <exec> <topology_file> <result_file> <drop queue limit(e.g. 100 for a limit of 100 packets)> <background data rate (e.g. 10 for 10 Mbps) <foreground-app-size (e.g. 100 for 100KB)>"<<endl;
      exit(0);
   }
   string topology_filename = argv[1];
   string result_filename = argv[2];
   int drop_queue_limit = atoi(argv[3]);
   string background_datarate = argv[4];
   string foreground_app_size = argv[5];
   int cs_clients = atoi(argv[6]);
   int cs_servers = atoi(argv[7]);
...
//
   ...
   int num_flows = 0; //total_host;
	ApplicationContainer background_app[num_flows];
	bool isClient[total_host];
	std::fill(isClient, isClient+total_host, false);
   srand(time(NULL));
   for(int i=0;i<num_flows; i++){
      ...
      //Initialize BulkSend background_application
         ...
         source.SetAttribute("MaxBytes",StringValue (maxBytes));
      }

      // Randomly select a client
      int client = rand() % total_host;

      while (client==i || isClient[client]){
         client = rand() % total_host;
      } // to make sure that client and server are different

      isClient[client] = true;
      int clientrack = topologyUtility::getHostRack(client);
      ...
      if(onOffApplication){
         NodeContainer onoff;
         onoff.Add(rackhosts[clientrack].Get(topologyUtility::getHostIndexInRack(client)));
         background_app[i] = oo.Install (onoff);
      ...
      //For bulk send
         ...
         bulksend.Add(rackhosts[clientrack].Get(topologyUtility::getHostIndexInRack(client)));
         background_app[i] = source.Install (bulksend);
         background_app[i].Start (Seconds (0.0));
         background_app[i].Stop (Seconds (simTimeInSec));
   ...
   cout<<"Finished creating background application"<<endl;
	...
	int num_clients = cs_clients; //
   int num_servers = cs_servers; //total_host/10;
   cout<<"CS Model, clients: "<<num_clients<<", servers: "<<num_servers<<endl;
   vector<int> clients, servers;
   /* Fill clients and servers vectors */
  
   int maxrackid = std::max_element(hostToTor.begin(), hostToTor.end(), hostToTor.value_comp())->second;
   ...
   std::fill(switch_covered, switch_covered + maxrackid, false);
	while(num_clients>0){
		...
		for (int i=0;i<hostsInTor[rack].size() && num_clients>0;i++){
			int svr = hostsInTor[rack][i];
			assert (rack == hostToTor[svr]);
			clients.push_back(svr);
			num_clients--;
		}
	}
	while(num_servers>0){
		...
		while(switch_covered[rack]) rack = rand()%maxrackid;
		switch_covered[rack] = true;
		for (int i=0;i<hostsInTor[rack].size() && num_servers>0;i++){
			int svr = hostsInTor[rack][i];
			assert (rack == hostToTor[svr]);
	...
	//for(int i=0; i<clients.size(); i++) cout<<"client "<<clients[i]<<endl;
	//for(int j=0; j<servers.size(); j++) cout<<"server "<<servers[j]<<endl;


	ApplicationContainer foreground_app[clients.size()][servers.size()];
	string app_bytes = foreground_app_size + "000"; // "0";		// unlimited
   cout<<"Foreground app size: "<<app_bytes<<"."<<endl;

   for(int c=0; c<clients.size(); c++){
      int csvr = clients[c];
      for(int s=0; s<servers.size(); s++){
         int ssvr = servers[s];
         char *add = topologyUtility::getHostIpAddress(csvr);
         BulkSendHelper source ("ns3::TcpSocketFactory", Address(InetSocketAddress(Ipv4Address(add), fg_port)));
         source.SetAttribute("MaxBytes",StringValue (app_bytes));
         int dstrack = topologyUtility::getHostRack(ssvr);
         NodeContainer bulksend;
         bulksend.Add(rackhosts[dstrack].Get(topologyUtility::getHostIndexInRack(ssvr)));
         foreground_app[c][s] = source.Install (bulksend);
         foreground_app[c][s].Start (Seconds (1.0));
         foreground_app[c][s].Stop (Seconds (simTimeInSec));
      }
   }
   std::cout << "Finished creating Bulk send foreground Application"<<"\n";
   ...
   for (int i=0;i<num_flows;i++){
	...
	for(int i=0; i<clients.size(); i++){
		for(int j=0; j<servers.size(); j++){
			foreground_app[i][j].Start (Seconds (1.0));
			foreground_app[i][j].Stop (Seconds (simTimeInSec));
		}
	}
